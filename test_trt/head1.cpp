...
int main(int argc, const char ** argv) 
{
    ...
    float fm[512* 6* 25* 25];
    for (int i = 0; i < 512* 6* 25* 25; i++)
        fm[i] = 1.0;
 
    float fq[512* 25* 25];
    for (int i = 0; i < 512*25* 25; i++)
        fq[i] = 1.0;
    ...
    cudaMemcpy(buffers[0], fm, batch_size * 512* 6* 25* 25* sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpy(buffers[1], fq, batch_size *512*25*25* sizeof(float), cudaMemcpyHostToDevice);
